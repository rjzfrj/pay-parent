...
import com.roncoo.pay.permission.biz.ZyppMenuBiz;
...
public class ZyppRoleController extends BaseController {
	...
	@Autowired
	private ZyppRoleService pmsRoleService;
	@Autowired
	private ZyppMenuService pmsMenuService;
	@Autowired
	private ZyppMenuRoleService pmsMenuRoleService;
	@Autowired
	private ZyppPermissionService pmsPermissionService;
	@Autowired
	...
	@RequestMapping("/list")
	public String listPmsRole(HttpServletRequest req, PageParam pageParam, ZyppRole pmsRole, Model model) {
		try {
	...
	public String addPmsRole(HttpServletRequest req, Model model, @RequestParam("roleCode") String roleCode, @RequestParam("roleName") String roleName, @RequestParam("remark") String remark, DwzAjax dwz) {
		try {
			ZyppRole roleNameCheck = pmsRoleService.getByRoleNameOrRoleCode(roleName, null);
			if (roleNameCheck != null) {
				return operateError("角色名【" + roleName + "】已存在", model);
			}

			ZyppRole roleCodeCheck = pmsRoleService.getByRoleNameOrRoleCode(null, roleCode);
			if (roleCodeCheck != null) {
				return operateError("角色编码【" + roleCode + "】已存在", model);
			}

			// 保存基本角色信息
			ZyppRole pmsRole = new ZyppRole();
	 ...
	 * @return msg .
	 */
	private String validatePmsRole(ZyppRole pmsRole) {
	...
	public String editPmsRoleUI(HttpServletRequest req, Model model, Long roleId) {
		try {
			ZyppRole pmsRole = pmsRoleService.getDataById(roleId);
			if (pmsRole == null) {
				return operateError("获取数据失败", model);
			}
	...
	@RequiresPermissions("pms:role:edit")
	@RequestMapping("/edit")
	public String editPmsRole(HttpServletRequest req, Model model, ZyppRole role, DwzAjax dwz) {
		try {
			Long id = role.getId();

			ZyppRole pmsRole = pmsRoleService.getDataById(id);
			if (pmsRole == null) {
				return operateError("无法获取要修改的数据", model);
			}

			ZyppRole roleNameCheck = pmsRoleService.getByRoleNameOrRoleCode(role.getRoleName(), null);
			if (roleNameCheck != null) {
				return operateError("角色名【" + role.getRoleName() + "】已存在", model);
			}

			ZyppRole roleCodeCheck = pmsRoleService.getByRoleNameOrRoleCode(null, role.getRoleCode());
			if (roleCodeCheck != null) {
	...
	public String deletePmsRole(HttpServletRequest req, Model model, Long roleId, DwzAjax dwz) {
		try {
			...
			ZyppRole role = pmsRoleService.getDataById(roleId);
			if (role == null) {
	...
	public String assignPermissionUI(HttpServletRequest req, Model model, Long roleId) {
		...
		List<ZyppPermission> permissionList = pmsPermissionService.listAll();
		List<ZyppOperator> operatorList = pmsOperatorRoleService.listOperatorByRoleId(roleId);
