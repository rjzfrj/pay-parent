...
public class ZyppOperatorController extends BaseController {
	...
	@RequestMapping("/list")
	public String listPmsOperator(HttpServletRequest req, PageParam pageParam, ZyppOperator operator, Model model) {
		try {
	...
	public String viewPmsOperatorUI(HttpServletRequest req, Long id, Model model) {
		try {
			ZyppOperator pmsOperator = pmsOperatorService.getDataById(id);
			if (pmsOperator == null) {
			...
			// 准备该用户拥有的角色ID字符串
			List<ZyppOperatorRole> lisPmsOperatorRoles = pmsOperatorRoleService.listOperatorRoleByOperatorId(id);
			StringBuffer owenedRoleIdBuffer = new StringBuffer("");
			for (ZyppOperatorRole pmsOperatorRole : lisPmsOperatorRoles) {
	...
	@RequiresPermissions("pms:operator:add")
	@RequestMapping("/add")
	public String addPmsOperator(HttpServletRequest req, ZyppOperator pmsOperator, @RequestParam("selectVal") String selectVal, Model model, DwzAjax dwz) {
		try {
			...
			// 校验操作员登录名是否已存在
			ZyppOperator loginNameCheck = pmsOperatorService.findOperatorByLoginName(pmsOperator.getLoginName());
	 ...
	 * @return
	 */
	private String validatePmsOperator(ZyppOperator operator, String roleOperatorStr) {
...
//		msg += loginPwdMsg;
	...
	@RequiresPermissions("pms:operator:edit")
	@RequestMapping("/edit")
	public String editPmsOperator(HttpServletRequest req, ZyppOperator operator, String selectVal, Model model, DwzAjax dwz) {
		try {
	...
	public String changeOperatorStatus(HttpServletRequest req, ZyppOperator operator, Model model, DwzAjax dwz) {
		try {
			...
			ZyppOperator pmsOperator = pmsOperatorService.getDataById(operatorId);
			if (pmsOperator == null) {
	...
	public String resetOperatorPwdUI(HttpServletRequest req, Long id, Model model) {
		ZyppOperator operator = pmsOperatorService.getDataById(id);
		if (operator == null) {
