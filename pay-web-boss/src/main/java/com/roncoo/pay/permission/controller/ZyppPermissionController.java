...
public class ZyppPermissionController extends BaseController {
	...
	@RequestMapping("/list")
	public String listPmsPermission(HttpServletRequest req, PageParam pageParam, ZyppPermission pmsPermission, Model model) {
		try {
	...
	@RequiresPermissions("pms:permission:add")
	@RequestMapping("/add")
	public String addPmsPermission(HttpServletRequest req, ZyppPermission pmsPermission, Model model, DwzAjax dwz) {
		try {
			...
			// 检查权限名称是否已存在
			ZyppPermission checkName = pmsPermissionService.getByPermissionName(permissionName);
			if (checkName != null) {
				return operateError("权限名称【" + permissionName + "】已存在", model);
			}
			// 检查权限是否已存在
			ZyppPermission checkPermission = pmsPermissionService.getByPermission(permission);
	 ...
	 * @return msg .
	 */
	private String validatePmsPermission(ZyppPermission pmsPermission) {
	...
	public String editPmsPermissionUI(HttpServletRequest req, Long id, Model model) {
		try {
			ZyppPermission pmsPermission = pmsPermissionService.getDataById(id);
	...
	@RequiresPermissions("pms:permission:edit")
	@RequestMapping("/edit")
	public String editPmsPermission(HttpServletRequest req, ZyppPermission permission, Model model, DwzAjax dwz) {
		try {
			...
			if (pmsPermission == null) {
				...
				// 检查权限名称是否已存在
				ZyppPermission checkName = pmsPermissionService.getByPermissionNameNotEqId(permissionName, id);
				if (checkName != null) {
					return operateError("权限名称【" + permissionName + "】已存在", model);
				}
	...
	public String deletePmsPermission(HttpServletRequest req, Long permissionId, Model model, DwzAjax dwz) {
		try {
			ZyppPermission permission = pmsPermissionService.getDataById(permissionId);
			if (permission == null) {
				return operateError("无法获取要删除的数据", model);
			}
			// 判断此权限是否关联有角色，要先解除与角色的关联后才能删除该权限
			List<ZyppRole> roleList = pmsRoleService.listByPermissionId(permissionId);
